...
if __package__ in {None, ""}:
    ...
    from backend.modules.access_control import authenticate, issue_token, permissions_for, verify_token
    from backend.modules.legal_gate import LEGAL_DOCUMENT_VERSION, create_acceptance, legal_terms_payload, receipt_payload
    from backend.modules.order_intake import ORDER_STATUSES, OrderIntake
    from backend.modules.tax_fee_settings import TaxFeeSettings, normalize_tax_fee_settings
else:
    from .storage import DATA_DIR, append_audit, ensure_data_files, read_json, write_json
    from .modules.access_control import authenticate, issue_token, permissions_for, verify_token
    from .modules.legal_gate import LEGAL_DOCUMENT_VERSION, create_acceptance, legal_terms_payload, receipt_payload
    from .modules.order_intake import ORDER_STATUSES, OrderIntake
    from .modules.tax_fee_settings import TaxFeeSettings, normalize_tax_fee_settings
...
class AuroraDeliHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/legal-terms":
            self.send_json({"legal_terms": legal_terms_payload()})
            return

        if parsed.path == "/api/legal-acceptance/status":
            query = parse_qs(parsed.query)
            acceptance_id = query.get("acceptance_id", [""])[0]
            self.send_json({"legal_acceptance": self.legal_acceptance_status(acceptance_id)})
            return

        if parsed.path == "/api/legal-acceptance/receipt":
            query = parse_qs(parsed.query)
            acceptance_id = query.get("acceptance_id", [""])[0]
            self.send_legal_receipt(acceptance_id)
            return

        if parsed.path == "/api/orders":
            if not self.require_permission("orders:read"):
                return
            self.send_json({"orders": read_json("orders")})
            return

        if parsed.path == "/api/orders-dashboard":
            if not self.require_permission("orders:read"):
                return
            self.send_json({"dashboard": self.build_orders_dashboard()})
            return
        ...
        if parsed.path == "/api/tax-settings":
            if not self.require_permission("tax_settings:read"):
                return
            self.send_json({"tax_settings": TaxFeeSettings(read_json("tax_fee_settings")).public_payload()})
            return
    ...
    def do_POST(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path == "/api/business-login":
            payload = self.read_json_body()
            if payload is None:
                return
            self.business_login(payload)
            return

        if parsed.path == "/api/legal-acceptance":
            payload = self.read_json_body()
            if payload is None:
                return
            self.record_legal_acceptance(payload)
            return

        if parsed.path == "/api/legal-decline":
            payload = self.read_json_body()
            if payload is None:
                return
            self.record_legal_acceptance(payload, accepted=False)
            return
        ...
        if parsed.path == "/api/order-estimate":
            payload = self.read_json_body()
            if payload is None:
                return
            if not isinstance(payload, dict):
                self.send_json({"error": "Order estimate body must be an object"}, HTTPStatus.BAD_REQUEST)
                return
            try:
                estimate = OrderIntake(read_json("products"), read_json("tax_fee_settings")).estimate(payload)
    ...
    def do_PATCH(self) -> None:
        ...
        if parsed.path.startswith("/api/orders/"):
            if not self.require_permission("orders:update"):
                return
            order_id = parsed.path.rsplit("/", 1)[-1]
            payload = self.read_json_body()
            if payload is None:
            ...
            if "status" in updates and updates["status"] not in ORDER_STATUSES:
                self.send_json({"error": f"Status must be one of: {', '.join(sorted(ORDER_STATUSES))}"}, HTTPStatus.BAD_REQUEST)
                return
        ...
        if parsed.path == "/api/settings":
            if not self.require_permission("settings:update"):
                return
            payload = self.read_json_body()
            if payload is None:
                return
        ...
        if parsed.path == "/api/tax-settings":
            if not self.require_permission("tax_settings:update"):
                return
            payload = self.read_json_body()
            if payload is None:
                return
            if not isinstance(payload, dict):
                self.send_json({"error": "Tax settings body must be an object"}, HTTPStatus.BAD_REQUEST)
                return
            try:
                settings = normalize_tax_fee_settings(
                    {**payload, "updated_at": datetime.utcnow().replace(microsecond=0).isoformat() + "Z"},
                    read_json("tax_fee_settings"),
                )
            except ValueError as exc:
                self.send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST)
                return
            write_json("tax_fee_settings", settings)
            append_audit("Tax settings", "tax_settings.update", "Updated order tax and fee settings.")
            self.send_json({"tax_settings": settings})
            return
    ...
    def require_permission(self, permission: str) -> dict[str, object] | None:
        ...
        if not auth:
            self.send_json({"error": "Business login required"}, HTTPStatus.UNAUTHORIZED)
            return None
        role = str(auth.get("role", ""))
        if permission not in permissions_for(role):
            self.send_json({"error": "Role is not allowed to access this resource"}, HTTPStatus.FORBIDDEN)
            return None
        return auth

    def business_login(self, payload: object) -> None:
        if not isinstance(payload, dict):
            self.send_json({"error": "Login body must be an object"}, HTTPStatus.BAD_REQUEST)
            return
        role = str(payload.get("role", "")).strip().lower()
        access_code = str(payload.get("access_code", "")).strip()
        user = authenticate(read_json("business_users"), role, access_code)
        if not user:
            self.send_json({"error": "Invalid business login"}, HTTPStatus.UNAUTHORIZED)
            return
        username = str(user.get("username", role))
        token = issue_token(role, username)
        self.send_json(
            {
                "session": {
                    "token": token,
                    "role": role,
                    "username": username,
                    "display_name": user.get("display_name", role.title()),
                    "permissions": sorted(permissions_for(role)),
                }
            }
        )

    def legal_acceptance_status(self, acceptance_id: str) -> dict[str, object]:
        if not acceptance_id:
            return {
                "accepted": False,
                "legal_document_version": LEGAL_DOCUMENT_VERSION,
            }
        for record in read_json("legal_acceptances"):
            if record.get("id") == acceptance_id and record.get("legal_document_version") == LEGAL_DOCUMENT_VERSION:
                return {
                    ...
                    "name": record.get("owner_full_name") or record.get("name"),
                    "business_name": record.get("business_name"),
                    "owner_email": record.get("owner_email"),
                    "phone": record.get("phone"),
                    "business_website": record.get("business_website"),
                    "timestamp_utc": record.get("timestamp_utc") or record.get("timestamp"),
                    "timestamp": record.get("timestamp_utc") or record.get("timestamp"),
    ...
    def record_legal_acceptance(self, payload: object, accepted: bool = True) -> None:
        if not isinstance(payload, dict):
            self.send_json({"error": "Legal acceptance body must be an object"}, HTTPStatus.BAD_REQUEST)
            return
        try:
            ...
            self.send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST)
            return
        records = read_json("legal_acceptances")
        records.append(acceptance)
        write_json("legal_acceptances", records)
        action = "legal.acceptance" if accepted else "legal.decline"
        verb = "Accepted" if accepted else "Declined"
        append_audit("Legal gate", action, f"{verb} {acceptance['legal_document_version']} for {acceptance['business_name']}.")
        self.send_json({"legal_acceptance": self.legal_acceptance_status(acceptance["id"])}, HTTPStatus.CREATED)

    def send_legal_receipt(self, acceptance_id: str) -> None:
        if not acceptance_id:
            self.send_json({"error": "acceptance_id is required"}, HTTPStatus.BAD_REQUEST)
            return
        for record in read_json("legal_acceptances"):
            if record.get("id") == acceptance_id:
                ...
                self.send_header("Content-Disposition", f"attachment; filename={acceptance_id}-legal-receipt.json")
                self.send_header("Content-Length", str(len(content)))
                self.end_headers()
                self.wfile.write(content)
                return
        self.send_json({"error": "Legal acceptance not found"}, HTTPStatus.NOT_FOUND)
    ...
    def create_order(self, payload: object) -> dict[str, object] | None:
        ...
        try:
            return OrderIntake(
                read_json("products"),
                read_json("tax_fee_settings"),
            ).create_order(payload, existing_count=len(read_json("orders")))
        except ValueError as exc:
            self.send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST)
            return None
    ...
    def build_orders_dashboard(self) -> dict[str, object]:
        orders = read_json("orders")
        buckets = {status: [] for status in sorted(ORDER_STATUSES)}
        for order in orders:
            status = str(order.get("status", "new"))
            if status not in buckets:
                status = "new"
            buckets[status].append(order)
        return {
            "counts": {status: len(items) for status, items in buckets.items()},
            "orders_by_status": buckets,
            "status_order": ["new", "accepted", "ready", "completed", "canceled", "rejected"],
        }
